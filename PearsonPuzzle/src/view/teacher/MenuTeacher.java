...
public class MenuTeacher extends Menu {
	...
	if (this.getComponentCount() <= menuItems.size()) {
	    ...
	    if (model.getProjectCode() != null
		    && !model.getProjectCode().isEmpty())
	    ...
	    if (model.getGroupMatrix() != null
		    && !model.getGroupMatrix().isEmpty()) {
		buttonBuffer.setForeground(containsSomething);
	    ...
	    if (model.getJUnitCode() != null && !model.getJUnitCode().isEmpty()
		    && model.getJUnitCode() != UnitEditor.DEFAULT_UNIT_CODE) {
		buttonBuffer.setForeground(containsSomething);
	    }
	...
	this.add(mainMenu);
	this.add(classMenu);
	this.add(configMenu, Component.RIGHT_ALIGNMENT);
	int seperator = 0;
	...
	if (extendedNavigation.size() >= navigationIndex) {
	    ...
	    extendedNavigation.get(navigationIndex).setBackground(
		    Color.LIGHT_GRAY);
	}
    }
