...
public class CompileDialog extends JDialog implements Observer {
	...
	if (model.getCompileFailures().size() == 0)
	    messageBuffer
		    .append("<span style=\"color: green;\">Kompilieren war erfolgreich.</span></p></body></html>");
	else {
	    messageBuffer
		    .append("<span style=\"color: red;\">Kompilieren war nicht erfolgreich.</span></p>");
	    for (HashMap<String, String> failure : model.getCompileFailures()) {
		messageBuffer.append("<div style=\"margin-left:20px;\">"
			+ failure.get("Art") + ": " + failure.get("Nachricht")
			+ " in Zeile " + failure.get("Zeile") + " von "
			+ failure.get("Klasse") + "</div>");
	    }
