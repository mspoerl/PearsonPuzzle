...
public abstract class JView implements Observer, View {
    ...
    protected static final Color DEFAULTBUTTONCOLOR = (new JButton())
	    .getBackground();
    protected static final ImageIcon saveIcon = new ImageIcon(
	    "rsc/icon/file/save_blue.png");

    // BorderLayout mit 5 horizontalem und 1 vertikalem Versatz zwischen den
    // Komponenten
     ...
     * als konkreter Controller für einen konkreten View hinzuzufügen. Action
    ...
    /**
     * Methode, um das Fenster aufzusetzen ung grundlegende Fenstereinstellungen
     * festzulegen. <br>
     * Wird nur zu Beginn aufgerufen, wenn ein neues Fesnter erstellt werden
     * soll, dies ist <br>
	...
	if (exception.getMessage() == PPException.databaseIsEmpty
	    ...
	    int n = JOptionPane
		    .showOptionDialog(
			    ...
			    "Datenbank wählen", JOptionPane.OK_CANCEL_OPTION,
	...
	case ShowHelp:
	    ...
	    break;
	case EditOrderGroup:
	    dialog = new EditOrderDialog(frame, model,
		    "Gruppe expliziter beschreiben");
	    break;
	case Compile:
	    dialog = new CompileDialog(frame, model,
		    "Ergebnis des Kompiliervorgangs");
	    break;
	default:
	    dialog = null;
	    break;
