...
public class dbTransaction implements Transaction {
	...
	try {
	    ...
	    if (((SQLException) e).getSQLState().equals("XJ040")) { // Failed to
								    // start
								    // database
								    // '<databaseName>',
								    // see the
								    ...
								    // for
								    // details.
		PPException exception = new PPException(
			PPException.anotherInstanceIsRunnign);
		throw exception;
	    } else if (e.getSQLState().equals("XJ004")) { // Database
							  // '<databaseName>'
							  // not found.
		PPException exception = new PPException(
			PPException.noDatabaseExists);
		throw exception;
	    } else if (e.getSQLState().equals("42X05")) { // Table/View
							  // '<objectName>' does
							  // not exist.
		PPException exception = new PPException(
			PPException.noDatabaseExists);
	...
	try {
		...
		throw new PPException(
			"<html>Nutzername existiert bereits. <br> Es können keine zwei Nutzer mit gleichem Namen angelegt werden.</html>");
	    }
	} catch (SQLException e) {
	    // Von doesUserExist geworfene Exception:
	    if (e.getSQLState().equals("XSCB1") // Container <containerName> not
						// found.
		    || e.getSQLState().equals("42X05")) { // Table/View <table>
							  // does not exist.
		try {
	...
	try {
		...
		try {
		    ...
		    // "Konnte Datenbank nicht erstellen.",
		    // "Ernsthaftes Problem aufgetreten!",
	...
	while (alreadyexists) {
		...
		Randomname = Randomname + (char) ('a' + 26 * Math.random()); // 'a'
									     // +
									     // (0..23)
									     // =
									     // ('a'
    ...
    public void saveProject(String projectname, String codeString,
	    String onlineimports, String localimports, String description,
	    int tab, Vector<Integer> lineOrder) {
	// TODO: Linelength umdefinieren (wird nicht zwingend benötigt,
	// übergebene Integer kann aber evtl. Verwendung finden
	...
	if (projectname == null || codeString == null) {
	    ...
	    projectname = new String(projectname.substring(0,
		    length_projectName - 1));
	} else if (description.length() > length_projectDescription) {
	    description = new String(description.substring(0,
		    length_projectDescription - 1));
	}

	// -- Code Array wird erzeugt und wieder zu einem String zusammengefasst
	codeString = unite(codeStrings, false, tab);

	// -- Eine zufällige Abfolge von Zahlen wird für die Sortierung der
	// Zeilen hinterlegt.
	...
	try {
	    // userDBaccess.saveProject(projectname, codeStrings, imports,
	    // description, randomKey, tab, linelength); 13.01.2017
	    userDBaccess.saveProject(projectname, createRandomString(15),
		    codeStrings, description, randomKey, tab, linelength);
	} catch (SQLException e) {
	    // TODO Auto-generated catch block
	    e.printStackTrace();
	}
	...
	if (jUnitCode.length() > 1000)
	    try {
		throw new PPException(
			"JUnit Code ist zu lang und konnte deshalb nicht gespeichert werden!");
	    } catch (PPException e) {
		e.printStackTrace();
		return;
	    }
	...
	try {
	    ...
	    if (e.getSQLState().equals("42X04")) { // Table/View '<objectName>'
						   // does not exist.
		return null;
	    }
	    e.printStackTrace();
    ...
    public void saveImports(String projectname,
	    HashMap<String, String> importMap) {

	userDBaccess.saveImports(projectname, importMap.get("online"),
		importMap.get("classes"), importMap.get("methods"));
    }
    ...
    public boolean setRandomKeys(String projectname,
	    LinkedList<Integer> sortedKeys) {
	...
	for (Integer indexInNormalCode : sortedKeys) {
	    permutation.set(indexInNormalCode,
		    sortedKeys.indexOf(indexInNormalCode));
	}
	// Vector<Integer> originalRandomKeys =
	// userDBaccess.getRandomKeys(randomname);
    ...
    // public void saveProject(String[] codeString, String projectname, int
    // linelength, final int projectID) throws SQLException{
    ...
    // }
    // userDBaccess.saveProject(codeString, mixedcodeString, projectname,
    // linelength, projectID);
    //
    // //-----------------------------testing------------------------------------
    //
    // }
    ...
    //
    // public String[] getRandomCodeArray(String projectname) throws
    // SQLException{
    ...
    public ArrayList<String> getCodeList(String projectname)
	    throws SQLException {
	projectname = getRandomName(projectname); // 13.1.2017
	...
	try {
		...
		try {
		    throw new PPException(
			    "Dieses Projekt enthält keinen Inhalt");
		} catch (PPException e1) {
		}
	    }
	    e.printStackTrace();
	...
	for (int ordernumber = 0; !userDBaccess.getOrder(projectname,
    ...
    public void saveOrder(String projectname,
	    Vector<Vector<Integer>> orderMatrix) {
	projectname = getRandomName(projectname); // 13.1.2017
    ...
    //
    // private void addOrder(String projectname, Vector<Vector<Integer>>
    // newOrdervektor){
	...
	try {
		...
		if (e.getSQLState().equals("42X05")) {
		    ...
		    PPException pEx = new PPException(
			    PPException.databaseIsEmpty);
	...
	for (int index = 0; index < orderFailureText.size(); index++) {
	    userDBaccess.addOrderfailurMassage(failurname, index,
		    orderFailureText.get(index));
	}
    }

    public boolean updateOrderFailure(String projectname, int ordernumber,
	    String orderFailureText) {
	String randomname = getRandomName(projectname);
	String failurname = randomname + "ORDERFAILURE";
	return userDBaccess.updateOrderfailurMassage(failurname, ordernumber,
		orderFailureText);
    }
	...
	String randomname = getRandomName(projectname);
	String failurname = randomname + "ORDERFAILURE";
	for (int ordernumber = 0; userDBaccess.doesOrderExist(randomname,
	...
	for (int index = 0; index < dataNames.size(); index++) {
		...
		if (tableName.length() == 16 || tableName.length() == 17) { // case1:
									    // eine
									    // Project-tabelle
		    ordercount = Integer.parseInt(tableName.substring(15));
		    String realtableName = tableName.substring(0, 15);
		    userDBaccess.createProject(realtableName, codeString,
			    randomKeys, 0);
		    for (int ord = 0; ord < ordercount; ord++) {
			userDBaccess.addOrder(realtableName,
				new Vector<Integer>());
		    }
...
// // Failed to start database '<databaseName>', see te next exception for
// details
// PPException exception = new PPException(
// PPException.anotherInstanceIsRunnign);
// throw exception;
...
// -----------------------------------------------------------------------------
//
// private static String unite(final String[] codeStrings, final boolean random,
// final int tab) {
// if (random) {
// String buffer;
// for (int i = codeStrings.length - 1; i > 0; i--) {
...
// }
//
// private static ArrayList<Integer> getRandomKeys(final int number) {
// ArrayList<Integer> randomKey = new ArrayList<Integer>();
// int array[] = new int[number];
// for (int j = 0; j < number; j++) {
// array[j] = j;
...
// userDBaccess.importTable(tableName, diskplace);
// } else {
// if (tableName.length() == 15) { // case1: eine Project-tabelle
// userDBaccess.createProject(tableName, codeString,
// randomKeys, 0);
// } else { // case1: eine Orderfailre-tabelle
// try {
// userDBaccess.createOrderfailurMassage(tableName);
