...
public class Model extends Observable {
    ...
    // Bei einer LinkedHaspMap ist auch die Reihenfolge der eingefügten Elemente
    // einsehbar.
     ...
     * @param integer
     *            Integer
     * @param integer_to_compare
     *            Zu vergelichende Integer
     */
    private boolean setChanged(final Integer integer,
	    final Integer integer_to_compare) {
    ...
    /**
     * Setzt den Projectcode. Leere Zeilen werden dabei entfernt.
     * 
     * @param codeString
     */
     ...
     * Nur TEACHER kann sowohl einen randomisierten, als auch einen normal
     * sortierten Vektor anfordern. Standardmäßig bekommt TEACHER einen nicht
     * randomisierten Vektor. Von STUDENT kann nur der ranomisierte Vektor
     * abgefragt werden.
     * 
     * @param random
     * @return
    ...
    /**
     * Gibt eine zewidimensionale Matrix zurück, die aus Vektoren besteht, die
     * ihrerseits eine aufsteigene Zahlenfolge beinhaltet. Jeder Vektor der
     * Matrix reprästiert eine Gruppe von Bedingungen für den Lösungsverktor.
     * 
     * @return
     */
    ...
    /**
     * Belegt den Eintrag an der Stelle x/y der Reihenfolgenmatrix mit dem Wert
     * value.
     * 
     * @param yPosition
     * @param xPosition
	...
	string = string.trim();
	try {
	    codeLine_GroupMatrix.get(xPosition).set(yPosition,
    ...
    // ---------------- Asserts können in der Zeile ergänzt werden (nicht
    // vollständig implementier)
	...
	for (Iterator<String> iterator = codeVector_random.iterator(); iterator
		.hasNext();) {
	    iterator.next();
    ...
    /**
     * Der Vektor sortedCode wird normalisiert, das heißt, dass vorher doppelt
     * vorhandene Einträge durch eindeutige Zuordnung ersetzt werden. Beispel: <br>
     * Zustand vor der Methode: codeVector_normal: ("a","a","b") codeMap: {"a"->
     * 0, "b"->2} sortedCode (0,0,2)<br>
     * Zustand nach der Methode: codeVector_normal: ("a","a","b") codeMap:
     * {"a"-> 0, "b"->2} sortedCode (0,1,2)
     */
    private void normalizeSortedCode() {
	for (int i = 0; i < codeVector_normal.size(); i++) {
	    if (!sortedCode.contains(i)
		    && sortedCode
			    .contains(codeMap.get(codeVector_normal.get(i)))) {
		try {
		    List<Integer> buffer = sortedCode.subList(
			    sortedCode.indexOf(codeMap.get(codeVector_normal
				    .get(i))) + 1, sortedCode.size());
		    int j = buffer
			    .indexOf(codeMap.get(codeVector_normal.get(i)));

		    // int j =
		    // sortedCode.lastIndexOf(codeMap.get(codeVector_normal.get(i)));
		    if (j >= 0)
			sortedCode
				.set(j + sortedCode.size() - buffer.size(), i);
		    // sortedCode.set(j,i);
		} catch (Exception e) {
		    // e.printStackTrace();
		}
    ...
    /**
     * Gibt eine Fehlerbeschreibung in Form einer HashMap zurück. TODO: Sind für
     * eine "OrderGroup" spezielle Ergebnisstrings in der Datenbank hinterlegt,
     * werden diese bei der zugehörigen Gruppe angehängt.
     * 
     * @return
     */
	...
	// Vector<String> codeVector_normal = (Vector<String>)
	// codeVector_normal.clone();
	// for(
	...
	if (groupFailures.size() == 0) {
	    result = OrderFailures.testOrder_simple(getSolutionStrings(),
		    codeVector_normal, true);
    ...
    /**
     * Gibt Aufschluss, ob die Lösungsmatrix die Codezeile enthält. Leerzeichen
     * und Tabs werden nicht berücksichtigt.
     * 
     * @param codeLine
     *            Codezeile, die geprüft werden soll
    ...
    /**
     * Dient dazu, Fehler zu hinterlegen, die beim Unit Testlauf aufgetreten
     * sind.
     * 
     * @param jUnitFailures
     *            the jUnitFailures to set
     */
    ...
    /**
     * Gibt einen leeren String (nicht null) zurück, wenn kein Unit Code
     * hinterlegt ist. Gibt Unit Sourcecode in Textform zurück, falls ein
    ...
    /**
     * JUnit Code wird ans Model übergeben. Dalls sich der übergebene Wert vom
     * im Model hinterlegten Wert unterscheidet, wird der Status des Models auf
     * "changed" gesetzt.
     * 
     * @param jUnitCode
     *            JUnit Sourcecode in Textform
     */
    ...
    /**
     * Speichert die im Model hinterlegten groben Eckdaten zum Projekt ab. Die
	...
	// normalizeSortedCode();
	// saveProject(getProjectCode(), getProjectName(),
	// getProjectDescription(), getSollution());
	// }
	notifyObservers(DCCommand.Save);
    }
     ...
     * wird projectName und der gewählte Eintrag ind projectList auf
     * Ungleichheit geprüft.
     * 
     * @param codeString
     *            Neuer Codeinhalt
     * @param projectName
	...
	// ----- Projekt speichern
	dataBase.saveProject(projectName, codeString, "", "",
		projectDescription, tabSize, null);
    ...
    /**
     * Löscht das mit <b>projectID</b> selektierte Projekt <br>
     * und löscht es aus der Datenbank <br>
    ...
    /**
     * Holt Projektbeschreibung <br>
     * <u>des aktuell in der Liste selektieren Projekts </u> <br>
     * aus der Datenbank
     */
    private void fetchProjectSettings() {
	if (projectID != null) {
	    try {
		this.projectDescription = dataBase
			.getProjectDescription(projectList.get(projectID));
    ...
    /**
     * Holt Projekt-Code <br>
     * <u>des aktuell in der Liste selektierten Projekts </u> <br>
     * aus der Datenbank
     */
	...
	if (projectID != null) {
	    ...
	    Vector<Integer> randomInts = dataBase
		    .getRandomKeys(getProjectName());
		...
		// Dies ist notwendig, damit im Text sort view die Tabs richtig
		// dargestellt werden.
		String tab;
		if (this.tabSize == 0)
		    tab = "";
	    ...
	    System.out.println("codeMap" + codeMap + "\trandom"
		    + codeVector_random + " " + randomInts);
	}
    }
	...
	if (accessGroup == null)
	    ...
	    if (dataBase.addUser(accessGroup.toString(), userName, new String(
		    passWord))) {
		notifyObservers(DCCommand.Save);
		return true;
	    }
    ...
    /**
     * Gibt eine Liste aller Nutzer der übergebenen Nutzergruppe zurück. Wenn
     * null übergeben wird, werden alle Nutzer zurückgegeben.
     * 
     * @param accessgroup
     *            Nutzergruppe
    ...
    /**
     * Setzt imports. Offiziell mögliche Typen sind: {methods, classes, online}<br>
     * Im Fall "online" müssen die imports in der klassischen Form: *import
     * paket.paket;* übergeben werden. <br>
     * Im Fall "methods" und "classes" müssen die Imports in Textform (als
     * Sourcecode) übergeben werden.
     * 
     * @param type
     *            {"methods", "classes", "online"}
     * @param text
     * @return
     */
     ...
     * Im Fall "online" sind die imports in der klassischen Form: *import
     * paket.paket;* hinterlegt. <br>
     * Im Fall "methods" und "classes" sind die Imports in Textform (als
     * Sourcecode) hinterlegt.
     * 
     * @param type
     *            {"online", "methods", "classes"}
     * @return Import
     */
    public String getImport(String type) {
	if (projectImports != null && projectImports.get(type) != null)
	...
	// da er im Prinip nur die Darstellung bestimmt und nicht im
	// eigentlichen Sinn zum Model gehört
	if (projectID != null)
    ...
    /**
     * Dient dazu, den Zugriff auf ein bestimmtes Prokjekt nur für einen
     * Bestimmten Studentennamen zu ermöglichen.
     * 
     * @param student
     */
    ...
    /**
     * Gibt einen Studentennamen zurück, falls dieses Projekt als nur für einen
     * Studenten sichtbar markiert ist. Gibt null zurück, falls das Projekt für
     * alle sichtbar ist.
     * 
     * @return
     */
