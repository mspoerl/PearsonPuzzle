 ...
 * den Implementierten Methoden i.A. dem Default Controller.
 * 
 * @author workspace
 */
public class AppletController implements Controller {
	...
	// Wechsel-----------------------------------
	case Login:
	    model.login((String) view.get("username"),
		    (char[]) view.get("password"));
	...
	case TestCode:
	    ...
	    if (model.getJUnitCode() != null
		    ...
		    && !model.getJUnitCode().equals(
			    UnitEditor.DEFAULT_UNIT_CODE)) { // Junit Test soll
							     // nur erfolgen,
							     // wenn auch
							     // einer
							     // definerit ist
		...
		try {
		    unitRunner = new UnitRunner(model.getJUnitCode(),
    ...
    /**
     * Methode instanziert neue Views. Etwaige vorher vorhandene Views werden
     * nicht entfernt. (Model hat weiterhin die alte View als Observer, View
     * diesen Controller als Controller). Ist gewünscht, dass alte Views
     * komplett verworfen werden, ist dies über dei Methode removeView() zu tun.
     * 
     * <ul>
     * <li>Übergebene View wird beim Model als Observer registriert</li>
     * <li>Das aktuelle RootPane erhält die übergebene View als Inhalt</li>
     * <li>View und Controller werden verknüpft</li>
     * <li>Die Menü Bar erhält die View, um sich eventuell anzupassen.
     * (Navigation etc.)</li>
     * <li>Das RootPane wird neu gezeichnet</li>
     * </ul>
     * 
     * @param view
     */
    ...
    /**
     * Entfernt die Bindung der Komponenten der aktuell gehaltenen View, sowie
     * die Bindung dieser View an das Model. ACHTUNG: Entfernt nicht die View
     * als Feld von Controller! (Um keine Null-Pointer-Exception zu riskieren)
     */
