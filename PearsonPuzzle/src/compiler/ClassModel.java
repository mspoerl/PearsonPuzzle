...
/**
 * Das ClassModel zerlegt Sourcecode in einzelne enthaltene Klassen, filtert
 * Klassennamen mithilfe von Code Completion heraus und fügt gewollte oder
 * notwendige Ergänzungen an (zusätzliche Methoden, Imports, Klassen).
 * 
 * <ul>
 * <li>Sollte der übergebene Sourcecode keine Klassendefinition enthalten, wird
 * er in eine Klasse "gewrapt".</li>
 * <li>Sollte der übergebene Sourcecode zudem kein Methodenkopf enthalten, wird
 * der Methodenrumpf "gewrapt". <br>
 * **Bsp: public void do(){ return;} -> public class <DEFAULT_CLASS_NAME> {
 * public void do( return;} }</li>
 * <li>Falls im Methodenrumpf ein return formuliert ist, wirde der Methode je
 * nach Bedarf der Rückgabewert Object oder void zugewiesen. <br>
 * **Bsp: int a; int b=0; a=b; return a; -> public int methode(){ int a; int b;
 * a=b; return a; }</li>
 * </ul>
 * 
 * @author workspace
...
public class ClassModel {
     ...
     * <ul>
     * <li>Falls in src keine Klasse definiert wird, wird der Code "gewrapt".
     * Dies geschieht, in dem eine Klasse namens <DEFAULT_CLASS_NAME> erstellt
     * wird, kommt dieser Name bereits in der srcCodeMap (als Key) oder in src
     * vor, werden Uterstriche angefügt, bis ein klassenname gefunden wurde, der
     * nicht bereits belegt ist.</li>
     * <li>Falls in src zusätzlich auch keine Methode deklariert ist, wird auch
     * diese "gewrapt". Dies geschieht, indem eine Methode namens
     * <DEFAULT_METHOD_NAME> erstellt wird. Kommt dieser Name bereits in src
     * vor, werden Unterstriche eingefügt, bis ein klassenname gefunden wurde,
     * der nicht bereits belegt ist.</li>
     * <li>Falls ein return enthalten ist, wird der Methode der Rückgabewert
     * Object oder void gegeben.</li>
     * </ul>
     * 
     * Konkret:
     * <ul>
     * <li>
     * Falls kein Klassenname existiert, wird versucht, den Sourcecode mit dem
     * unter DEFAULT_CLASS_NAME definierten Namen zu wrappen. Bsp: public void
     * do(){ return;} -> public class <DEFAULT_CLASS_NAME> { public void do(
     * return;} }</li>
     * <li>
     * Falls kein Methodenrumpf existiert, wird zusätzlich noch ein
     * Methodenrumpf dazugebaut, um das ganze kompilierbar zu machen. Bsp: int
     * a; int b=0; a=b; return a; -> public int methode(){ int a; int b; a=b;
     * return a; }</li>
     * </ul>
     */
    public ClassModel(String sourceCode, String methodImports,
    ...
    /**
     * Liefert eine Hash Map in der Form: Klassenname -> Sourcecode
     * 
    ...
    /**
     * Liefert eine Liste mit imports, die die Klasse benötigt. Wollen diese
     * verwendet werden, müssen die Einträge in der Collection im Stil <b>import
     * importName;</b> verwendet werden
     * 
     * @param className
     * @return importNames
     */
    ...
    /**
     * Falls der im Konstruktor übergebene sourceCode keine Klasse enthält, wird
     * eine solche erzeugt und der Code "gewrapt". Der Name der Klasse kann hier
     * abgerufen werden.
     * 
    ...
    /**
     * Fall der im Kostruktor übergebene sourceCode keine Methode enthält, wird
     * eine solche erzeugt und der Code "gewrapt". Der Name der Methode kann
    ...
    /**
     * Fügt online imports (import ...;) zu Beginn der Klasse an. Dabei wird auf
     * etwaige vorherige festgelegte packages geachtet.
     */
	...
	for (String codeName : srcCodeMap.keySet()) {
		...
		srcCodeMap.put(
			...
			srcCodeMap.get(codeName).substring(0, matcher.end())
				+ "\n"
				+ onlineImports.trim()
				+ srcCodeMap.get(codeName).substring(
					matcher.end()));
	    }
	}
    }
    ...
    /**
     * Gewünschte Methoden oder Feld-Definitionen werden am Ende !aller! zu
     * testenden Klassen eingefügt. (Beziehungsweise aller zu dieser Zeit in
     * srcCodeMap entahltenen Klassen) Es ist daher von entscheidender
     * Bedeutung, ob addMethods(String) vor oder nach addClasses(String)
     * ausgeführt wird.
     * 
     * @param methodImports
     */
    ...
    /**
     * HashMap sourceCodeMap wird mit Inhalt gefüllt.<br>
     * Falls in src keine Klasse definiert wird, wird der Code "gewrapt". Dies
     * geschieht, in dem eine Klasse namens <DEFAULT_CLASS_NAME> erstellt wird,
     * kommt dieser Name bereits in der srcCodeMap (als Key) oder in src vor,
     * werden Uterstriche angefügt, bis ein klassenname gefunden wurde, der
     * nicht bereits belegt ist.<br>
     * Falls in src zusätzlich auch keine Methode deklariert ist, wird auch
     * diese "gewrapt". Dies geschieht, indem eine Methode namens
     * <DEFAULT_METHOD_NAME> erstellt wird. Kommt dieser Name bereits in src
     * vor, werden Unterstriche eingefügt, bis ein klassenname gefunden wurde,
     * der nicht bereits belegt ist. <br>
     * Falls ein return enthalten ist, wird der Methode der Rückgabewert Object
     * gegeben.
     * 
     * @param src
     *            Sourcecode
	...
	if (!src.contains(" class ")) {
		...
		if (matcher_ret.find()) { // kein return Statement im Sourcecode
		    ...
		    // return statement im Sourcecode
	...
	// Falls der Code " class " enthält, werden diese Klassen
	// herausgefiltert.
	else {
    ...
    /**
     * HashMap sourceCodeMap wird mit Inhalt gefüllt. Tut nicht, wenn keine
     * codeLine mit " class " enthalten, spirch in codeLines keine Klasse
     * definiert ist. Imports und packages werden herausgefiltert und können
     * Klassenspezifisch verarbeitet werden.
     * 
     * @param codeLines
     */
	...
	for (String line : codeLines) {
		...
		if (packageString.isEmpty()) {
		    packageString = CodeCompletion.extractDeclarationName(line,
			    nextLine, "package");
		    // packageString = line.substring(line.indexOf("package")+7,
		    // line.indexOf(";"));
		    // packageString = packageString.trim();
		} else if (!className.isEmpty()) {
		    putClass(className, solutionString, packageString,
			    importStrings);
	    ...
	    } else if (CodeCompletion.extractDeclarationName(line, nextLine,
		    "import") != null) {
		// else if (line.trim().startsWith("import ")){
		if (className.isEmpty()) {
		    String importString = CodeCompletion
			    .extractDeclarationName(line, nextLine, "import");
		    // String importString =
		    // line.substring(line.indexOf("import")+6,
		    // line.indexOf(";"));
