...
/**
 * Klasse ermöglicht, Drag&Drop Aktionen durchzuführen. Für die Grundsätzliche
 * Arbeitsweise einer Drag and Drop Liste betrachte FromTransferHandler. Es
 * werden nur bekannte Elemente (aus codeVector) aufdgenommen. Um dies zu prüfen
 * greift die Klasse auf das Model zurück. Es kann aus verschiedenen Drag&Drop
 * Modi gewählt werden. Das Verhalten des Controllers kann zu Laufzeit mittels
 * enableRemove(); disableRemove(); sowie setAction(); verändert werden.
 * 
 * @author workspace
 * 
 */
public class ToSaveTransferHandler extends TransferHandler {
    ...
    private Integer dropIndex; // Listenindex der Position, an der ein Element
			       // gedropt wird.

    private int action; // COPY oder MOVE
    private DropMode defaultDropmode; // Damit externer DnD prinzipiell auch
				      // Elemnte ersetzten kann (bei internem
				      // DnD immer Insert)
    private boolean internDnD; // Gibt Auskunft, ob es sich um ein
			       // Listeninternes DnD Event handelt (wird beim
			       // Export gesetzt und beim Import ausgelesen)
    private boolean dragElements_infinitly; // Das gleiche Element kann
					    // unendlich oft importiert werden
    private boolean removeElements; // Elemente wandern wieder zurück
    private boolean deleteElements; // Elemente wandern nicht wieder zurück,
				    // sondern werden entfernt
    private Model model;

    public ToSaveTransferHandler(DefaultListModel<String> dropDList,
	    JList<String> dropJList, int Type, Model model) {
	this.model = model;
	switch (Type) {
	case 0:
	...
	// Hier wird die Anzahl der Elemente gleichen Namens (in CodeString und
	// in der DragList) gezählt
	int ocNumber_main = 0;
	...
	if (internDnD) {
	    ...
	    // Daten von Extern werden nicht anerkannt (Wenn die rechte Liste
	    // diese Daten nicht enthält, wird abgelehnt)
	    return false;
	else if (ocNumber_main <= ocNumber && !dragElements_infinitly) {
	    // Daten von extern werden nicht anerkannt (Anzahl der bereits
	    // vorkommenden Einträge wird ermittelt)
	    return false;
	}
	...
	internDnD = true; // Information, dass es sich um ein itnernes DnD
			  // handelt
	dropJList.setDropMode(DropMode.INSERT); // Elemente werden verschoben,
						// nicht ersetzt
