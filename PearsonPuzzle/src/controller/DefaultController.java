 ...
 * Superklasse JView gedacht. Es werden:
 * <ul>
 ...
 * @author workspace
 */
public class DefaultController implements Controller, TableModelListener,
	FocusListener {
	...
	if ((view.getClass().equals(TextEditor.class)
		|| view.getClass().equals(UnitEditor.class) || view.getClass()
		.equals(ConfigEditor.class))
		&& cmd != DCCommand.Save
		&& cmd != DCCommand.ConnectedComponent) {
	    if (view.getClass().equals(TextEditor.class)
		    && ((view.get("projectname") == null || ((String) view
			    .get("projectname")).trim().isEmpty()))
		    && (cmd == DCCommand.EditConfig
			    || cmd == DCCommand.EditJUnit || cmd == DCCommand.EditPreview)) {
	...
	case Save:
		...
		if (((TextEditor) view).getCode() == null
			...
			|| (((TextEditor) view).getProjectName()).trim()
				.equals("")) {

		    view.showDialog(Allert.noContentInput);
		} else {
		    // ---- Es wird versucht das Projekt zu speichern, schlägt
		    // dies fehl, so existiert bereits ein Projekt mit gleichem
		    // Namen
		    if (model.saveProject(((TextEditor) view).getCode(),
			    ((TextEditor) view).getProjectName(),
			    ((TextEditor) view).getProjectDescription(), 150)) {
		...
		if (((Component) e.getSource()).getName() != null
			&& ((Component) e.getSource()).getName().equals(
				"randomize")) {
		    model.saveProject(true);
		    // view.update(null, DCCommand.Save);
		} else if (((Component) e.getSource()).getName() != null
			&& ((Component) e.getSource()).getName().equals("sort")) {
		    model.savePuzzlemode(((PreViewEditor) view)
			    .getPuzzleModus());
		    model.saveRandomisation();
		} else if (((Component) e.getSource()).getName() != null
			&& ((Component) e.getSource()).getName().equals("save")) {
		    model.savePuzzlemode(((PreViewEditor) view)
			    .getPuzzleModus());
		}
	    }
	...
	case DeleteProject:
		...
		if (model.getProjectListID() == null)
			...
			view.update(null, DCCommand.DeleteProject);
		    }
		}
	    }
	    break;
	...
	case ConnectedComponent:
	    ...
	    // (((TextEditor)view).getInputComponents());
	    // model.setTabSize(Integer.parseInt(inputFields.get(0).getText()) %
	    // 10);
	...
	case Compile:
		...
		if (view.getClass().equals(UnitEditor.class)) {
		    UnitRunner unitRunner = new UnitRunner(
			    ((UnitEditor) view).getContent(),
			    model.getProjectCode(), model.getImport("methods"),
			    model.getImport("online"),
	...
	case TestCode:
		...
		if (model.getJUnitCode() != null
			&& !model.getJUnitCode().isEmpty()
			&& !model.getJUnitCode().equals(
				UnitEditor.DEFAULT_UNIT_CODE)) { // Junit Test
								 // soll nur
								 // erfolgen,
								 // wenn auch
								 // einer
								 ...
								 // ist
		    UnitRunner unitRunner = new UnitRunner(
			    model.getJUnitCode(), model.getSollution(),
			    model.getImport("methods"),
			    model.getImport("online"),
			    model.getImport("classes"));
		    // unitRunner.addOnlineImport(model.getImport("online"));
		    // unitRunner.addClasses(model.getImport("classes"));
	...
	case DB_Import:
		...
		model.replaceDatabase(file.getName(), file.getParentFile()
			.getAbsolutePath());
		// This is where a real application would open the file.
	    }
	    break;
    ...
    /**
     * XXX: Soll !eventuell! ins Modell ausgelagert werden Nutzername @param
     * username Passwort @param password
     */
	...
	if (((AbstractButton) e.getItem()).getActionCommand() == DCCommand.ResetDB
		.toString()) {
	    if (e.getStateChange() == ItemEvent.SELECTED && !model.isResetDB()) {
	...
	if (view.getClass() == UserEditor.class) {
		...
		model.setUserGroup_toEdit(AccessGroup.valueOf(((Component) e
			.getSource()).getName()));
		try {
		    act(DCCommand.EditUsers, null);
	...
	if (view.getClass().equals(TextEditor.class)
		...
		if (compName.equals("AccessGroup")) {
		    ...
		    JComboBox<String> jComboBox = (JComboBox<String>) e
			    .getSource();
		...
		if (columnName.equals("Codezeile")) {
			...
			if (rule < max && rule != 0) {
				...
				throw new PPException(
					"<html><body>Problem in Zeile "
						+ (1 + (Integer) row)
						+ ", Spalte "
						+ ((Integer) column + 1)
						+ "<BR> Die Einträge dürfen nicht der Vorgabe widersprechen.</body></html>");
			    } catch (PPException e1) {
