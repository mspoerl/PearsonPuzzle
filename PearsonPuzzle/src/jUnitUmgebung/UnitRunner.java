...
/**
 * Klasse hält jUnit Test und zugehörige, zu testende Klassen in Form von
 * Strings bereit, um diese bei Bedarf zu kompilieren oder JUnit Test zu
 * starten. Es können auch nur Mehtoden oder nur der Inhalt einer Methode (ohne
 * Methodenrumpf) übergeben werden. Diese werden druch das compiler.ClassModel
 * "gewrapt" <br>
 * <br>
 * 
 * <br>
...
public class UnitRunner {
     ...
     * Klasse hält jUnit Test und zugehörige, zu testende Klassen in Form von
     * Strings bereit, um diese bei Bedarf zu kompilieren oder JUnit Test zu
     * starten. Es können auch nur Mehtoden oder nur der Inhalt einer Methode
     * (ohne Methodenrumpf) übergeben werden. Diese werden druch das
     * compiler.ClassModel "gewrapt"
     * 
     * @param sourceCode_ToBeTested
     *            zu testende Klassen, Methoden, ...
     * @param imports
     *            evtl. notwendige imports
     * @throws PPException
     */
    public UnitRunner(String unitSourceCode, String sourceCode_ToBeTested,
	    String methodImports, String onlineImports, String classImports)
	    throws PPException {
	unitLineOffset = 0;
    ...
    /**
     * TODO: Line Offset implementieren (durch Löschen von Kommentaren oder
     * allgemein Änderungen am Source-Code kann es zu Zeilenversatz kommen,
     * dieser soll hier zurückgegeben werden)
     * 
     * @return
     */
    ...
    // * Falls ein return enthalten ist, wird der Methode der Rückgabewert
    // Object gegeben.
    // *
    ...
    // "(public|protected|native|private|static|\\s) +[\\w\\<\\>\\[\\]]+\\s+(\\w+) *\\([^\\)]*\\) *(\\{?|[^;])";
    ...
    // definiert ist.
    // * Imports und packages werden herausgefiltert und Klassenspezifisch
    // verarbeitet.
    // *
    // * @param codeLines
    // */
    ...
    // packageString = CodeCompletion.extractDeclarationName(line, nextLine,
    // "package");
    ...
    // String importString = CodeCompletion.extractDeclarationName(line,
    // nextLine, "import");
    // // String importString = line.substring(line.indexOf("import")+6,
    // line.indexOf(";"));
	...
	for (String className : srcCodeMap.keySet()) {
	    ...
	    JavaFileManager fileManager = new MemJavaFileManager(diagnostics,
		    compiler, classLoader);
	    JavaFileObject javaFile = new StringJavaFileObject(className,
		    sourceCode);
    ...
    /**
     * Unit Test wird kompiliert und gestartet. Anschließend werden vorhandene
     * Klassendateien gelöscht.
     * 
     * @param unitText
     *            JUnit Klasse in Textform
     * @return Result
     */
	...
	DiagnosticCollector<JavaFileObject> diagnostics = new DiagnosticCollector<JavaFileObject>();
	JavaFileManager unitFileManager = new MemJavaFileManager_JUnit(
		diagnostics, compiler, unitClassLoader, normalClassLoader);
	System.out.println(unitClassName);
	System.out.println(unitSourceCode);

	JavaFileObject unitFile = new StringJavaFileObject(unitClassName,
		unitSourceCode);
	...
	// options.add( "-verbose");
	// options.add("-deprecation");
	CompilationTask task = compiler.getTask(null, unitFileManager,
    ...
    /**
     * Führt die UnitTests, die im Konstruktor in einer Testklasse in Stringform
     * übergeben wurde aus und bindet die Klasse(n), die ebenfalls im
     * Construktor übergeben wurden, aus.
     * 
     * @return Result
     */
    public Result run() {
	...
	try {
	    Result result = JUnitCore.runClasses(unitClassLoader
		    .findClass(unitClassName));
