...
public class DBAccess {
	...
	for (String projectName : stringList) {
	    if (projectName != null
		    && projectName.length() > UserDBaccess
			    .getLength_ProjectName()) {
		projectName = new String(projectName.substring(0,
			UserDBaccess.getLength_ProjectName() - 1));
	    }
	...
	for (String projectName : stringList) {
		...
		try {
		    ...
		    // ----
		    if (codeString == null)
			// NotEqual, weil dann nichts gespeichert wurde (und
			// noch der Code aus dem letzten Schleifendurchlauf
			// abgerufen wird)
		    ...
		    // Manche Projektnamen sind nicht zulässig und werfen dann
		    // beim get eine Exception (weil kein zugehöriger Datensatz
		    // vorhanden ist)
		    if (projectName == null)
			continue;
		    if (projectName.equals(emptyString)
			    || projectName.equals(spaceString)
			    && e.equals("24000"))
			// SQL Error: Invalid cursor state - no current row. (
			continue;
		    e.printStackTrace();
		    fail("Problem beim vergleichen von " + codeString
			    + "... in " + projectName + "...");
		}
	    }
	}
    ...
    /*
     * 
     * private void saveProject(final String projectName, final String
     * codeString, final int linelength){ // TODO: Testfehler sauber definieren
     * db.saveProject(projectName, codeString, linelength, 0); try {
     * assertEquals(db.getCode(projectName),codeString); } catch (SQLException
     * e) { fail("Problem nicht spezifiziert"); // TODO Auto-generated catch
     * block e.printStackTrace(); } }
     * 
     * @Test public void multiThread(){ try { UserDBaccess database = new
     * UserDBaccess(); database.connectionToDerby(); } catch (SQLException e) {
     * fail("Keine zwei Verbingunen möglich"); e.printStackTrace(); } }
     * 
     * @Test public void test_Normal() { assertTrue(isEqual(normalString, "2",
     * normalString,150)); }
     * 
     * @Test public void test_Normal_noSplit() {
     * assertTrue(isEqual(normalString, "", normalString,150)); }
     * 
     * @Test public void test_Normal_Empty() { assertTrue(isEqual(normalString,
     * "2", emptyString,150)); }
     * 
     * @Test public void test_Empty_Normal() {
     * assertTrue(isEqual(emptyString,normalString,"2",150)); }
     * 
     * @Test//(expected=IllegalArgumentException.class) public void test_MIN() {
     * assertTrue(isEqual(normalString,"2",normalString,Integer.MIN_VALUE)); }
     * 
     * @Test//(expected=IllegalArgumentException.class) public void
     * test_Empty_Empty_MIN() {
     * assertTrue(isEqual(emptyString,"",emptyString,Integer.MIN_VALUE)); }
     * 
     * @Test//(expected=IllegalArgumentException.class) public void test_MAX() {
     * assertTrue(isEqual(normalString, "", normalString, Integer.MAX_VALUE)); }
     * 
     * @Test//(expected=IllegalArgumentException.class) public void
     * test_Tab_Tab() { assertTrue(isEqual(tabString, "", tabString, 150)); }
     * 
     * @Test//(expected=IllegalArgumentException.class) public void
     * test_NewLine_NewLine() { assertTrue(isEqual(newLines, "", newLines,
     * 150)); }
     * 
     * @Test public void test_Fancy() { assertTrue(isEqual(fancyString, "2",
     * fancyString, 150)); }
     * 
     * @Test public void test_Normal_Fancy() { assertTrue(isEqual(normalString,
     * "2", fancyString, 150)); }
     * 
     * @Test public void test_Fancy_Normal() { assertTrue(isEqual(fancyString,
     * "", normalString, 150)); }
     */
    /**
     ...
     * @return
     */
    private boolean isEqual(final String code, final String split_String,
	    final String projectname, final int linelength) {
	try {
	    db.saveProject(projectname, "", code.split(split_String), "",
		    new ArrayList<Integer>(), linelength, 0);
	} catch (SQLException e) {
